...
def is_valid_word(word, hand, word_list):
    ...
    for key in word_dict :
        if new_hand.get(key, 0) < word_dict[key] :
            return False
    if "*" in word :
        for vowel in VOWELS :
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while n >= 1 :
        ...
        if is_valid_word(word, hand, word_list) :
            ...
            score += word_score
            print("\"" + word + "\" has earned " + str(word_score) + " points. Hand score: " + str(score) + " points")
        else :
            print("That is not a valid word. Please choose another word.")
        hand = update_hand(hand, word)
        n = calculate_handlen(hand)
    if n == 0 :
        print("Ran out of letters. Hand score:", score)
    else :
...
def substitute_hand(hand, letter):
    ...
    while new_hand.get(new_letter) != None :
        new_letter = random.choice(list(SCRABBLE_LETTER_VALUES.keys()))
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while n_hands > 0 :
        ...
        print("Current hand: ")
        display_hand(hand)
        ask_substitute = input("Would you like to substitute a letter?(y/n)").lower()
        if ask_substitute == "y" :
            ask_letter = input("Which letter would you like to replace: ").lower()
            hand = substitute_hand(hand, ask_letter)
        hand_score = play_hand(hand, word_list)
        print("----------")
        if replay > 0 :
            ask_replay = input("Would you like to replay the hand?(y/n)").lower()
            if ask_replay == "y" :
                replay -= 1
